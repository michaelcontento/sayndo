...
var serverModule = {
   ...
   requestHandler: function (app, req, res) {
        ...
        session.readByCookie(reqCookies, function(properties, authType) {
            /*
            ...
            res.session.update = function(properties) {
                session.update(res.session.id, properties);
            }
    ...
    node: function(app) {
        ...
        httpServer.on('close', function(errno) {
            log.info('Close server with: ' + errno);
        });
    ...
    init: function() {
        ...
        // Flush riak to start with a fresh db.
        riak.flushAll();
