 ...
 * Clearly simple and more than 20 times faster than express.
 * But this is NOT a replacement for express. The express
 * framework supports a lot of cool functions, this server does not.
 */
var server = {
    /*
     * Create the http server object (slave node).
     *
     * Current Performance by running the single server:
     *  - > 4000 req/s
     *  - > 1500 bytes
     *  - ~ 23 ms/req
     *  - ~ 110 ms browser onload
     *  - views and files
     *  - http://sensual.com/
     *
     * Performance problems:
     *  - -1000 req/s with session support
     *  - -600 req/s with view local replacement
     */
    node: function(app) {
        ...
        server.on('request', function (req, res) {
            ...
            session.get(req.headers.cookie, function(session, authType) {
                req.session = session;
